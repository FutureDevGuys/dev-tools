...
Programmatic grant requests require an explicit approved duration within administrator policy; interactive approval may offer permitted short and overnight presets. There is no fixed product-wide eight-hour maximum. A request cannot widen policy's idle or hard cap, and the approved duration may narrow either. The shared lifecycle retains its historical `standard()` convenience values of 30 minutes idle and two hours hard, but those values do not authorize a product grant. Time accounting is monotonic and includes suspension where the native contract requires it. Only accepted useful activity resets idle time. Polling, heartbeats, and delegation never extend the approved hard expiry. Leases are memory-only and broker restart invalidates them.
