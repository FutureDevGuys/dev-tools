...
`SecretMaterial` takes ownership behind its zeroizing destructor before validating the input length, so rejected oversized provider bytes use the same erasure path as accepted material. The accepted size range and value-free error category are unchanged. This covers the owned allocation, not copies made before ownership transfer or historical allocator reallocations. Size-boundary and diagnostic-opacity tests do not inspect freed memory; rejection-path erasure additionally relies on the unconditional Rust drop guard and the audited `zeroize` implementation.
