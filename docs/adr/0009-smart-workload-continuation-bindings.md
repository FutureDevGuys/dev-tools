...
Continuation discovery retains absolute UTF-8 search-path entries that are currently absent, preserving their position while looking for an existing candidate. Missing optional tool directories do not abort discovery and are never created. Other canonicalization failures remain errors, relative entries remain rejected, and existing resolvable proxy layers remain excluded. A retained absent path is not executable identity, receipt ownership or approval of any future target that appears there.

Discovery also skips broken candidate symlinks and, on Unix, regular candidates with no executable permission bits, without changing either entry. Selection still requires the existing bounded, stable single-link executable inspection; this lookup correction does not relax selected-target custody, implement ACL-aware search or establish native Windows launch semantics. Linux fixtures compare these two skip cases with Bash command lookup before checking the resolved wrapper and preserved search cursor.
