...
The released native `sync-configs` executable is built from the workspace crate with `cargo build --release --locked --bin sync-configs`; target hosts do not need Python or a package resolver. Release-set construction reads the product version from Cargo metadata and signs one exact target-specific artifact under the independent `sync-configs/vX.Y.Z` tag. Windows artifacts use the `sync-configs.exe` executable name. Linux x86-64 is the only target admitted to the current cutover. The signed 0.2.0 generation passed isolated fresh installation without external tools, live upgrade from retained 0.1.11, no-change repeat installation, offline rollback and restoration. Consumer-owned convergence qualification and broader native runtime acceptance remain separate gates. The release builder, signer, publisher, and installer reject every other `sync-configs` runtime meanwhile.
...
Authentication failures distinguish the noninteractive cached check from the terminal-facing prompt. Diagnostics retain only the fixed process-error category and numeric OS code, or the prompt's exit code and signal; they do not retain executable paths, arguments, environment, subprocess output or original I/O error text. This improves failure localization without changing timestamp reuse, prompting, interruption precedence or credential custody. A successful cached check still suppresses the prompt, and failed authentication never marks the session authenticated.
