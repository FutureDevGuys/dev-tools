...
GC previews hold the existing coordination lock without creating it or removing stale activity records. A busy root defers or fails without waiting. If an older root has no coordination lock, explicitly run `dev-cache config init-root ROOT` for the configured root (retaining `--config PATH` when using a nondefault configuration) before previewing again. Initialization prepares coordination; a preview never does. Applied collection retains stale-record cleanup and recomputes its current plan rather than executing a previously printed list.
