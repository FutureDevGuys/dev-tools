...
The 0.1.8 candidate keeps setup-only lease metadata in memory and durably publishes the scoped record before unlocking. Repository identity, resource registration and completion writes retain their existing synchronization. Four lease-setup integration tests cover collector exclusion, abandonment, publication failure and independent same-process activities; the last case also prevents the previous PID/second filename collision. These source tests do not establish an installed speedup: release-level repeated timing and syscall comparison remain required before claiming performance acceptance for 0.1.8.
