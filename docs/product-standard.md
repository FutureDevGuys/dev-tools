...
New common update adapters use `dev-tools-operation-result-v2`; existing read-only doctor output retains `dev-tools-operation-result-v1` during the explicit transition in ADR 0019. The result identifies the product, operation, outcome, changed state, process exit category, fixed value-free error kind, managed or external installation state, cache freshness, and installed or available versions when applicable. For updates, `changed` concerns durable managed installation state, including recovery records, rather than disposable cache or release-observation writes. V2 reports true for established change, false for established no change, and null when entered mutation has uncertain progress. A post-mutation installed identity comes from a fresh read-only observation; observation failure leaves identity unknown without erasing established change. V1's boolean field and existing public Rust type remain unchanged; null is never emitted under the v1 schema.
