...
For one product and target's acceptance state, an already accepted exact version cannot acquire a different artifact digest through a higher manifest generation. Metadata and root generations may advance while retaining the same version and artifact bytes. Products must keep acceptance state bound to their product and target; the shared state-transition primitive does not infer that ownership.
