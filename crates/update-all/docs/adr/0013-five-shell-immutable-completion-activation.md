...
Snapshot retention is part of the completion store rather than a daemon. The default policy protects the current snapshot, three prior snapshots, and every prior snapshot younger than 24 hours; `completions.retention_prior_snapshots` and `completions.retention_min_age_hours` configure those floors. Pruning may run only after `current` changes through a successful `published` activation, every selected provider reports a complete inventory, and no candidate is probed-unchanged, retained, unsupported, or failed. It does not run for repaired or unchanged activation. Before any deletion, the store proves the exact owned shape of every historical snapshot and verifies content-addressed object files; ambiguity defers the entire cleanup pass. A clean pass removes only eligible historical snapshots and objects unreferenced by the current or protected snapshots.
