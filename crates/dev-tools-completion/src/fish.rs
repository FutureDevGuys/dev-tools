// Adapted from clap_complete 4.6.9, src/aot/shells/fish.rs.
// Copyright clap contributors. Licensed under MIT OR Apache-2.0.
// Upstream: https://github.com/clap-rs/clap
// Local changes: retain generation below the upstream two-subcommand cutoff;
// render built metadata into a String without the upstream panicking I/O adapter.
// Remove this module when the native deep-tree regression passes upstream.
...
pub(super) fn render(cmd: &Command, bin_name: &str) -> String {
    ...
    if cmd.has_subcommands() {
        gen_subcommand_helpers(&name, cmd, &mut buffer, needs_fn_name, using_fn_name);
    } else {
