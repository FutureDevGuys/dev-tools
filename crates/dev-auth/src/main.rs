...
fn usage() -> &'static str {
    "Usage:\n  dev-auth exec --profile NAME -- /absolute/command [args...]\n  dev-auth ssh-load --profile NAME\n  dev-auth status\n  dev-auth purge"
}
...
fn run() -> Result<i32> {
    ...
    match command.as_str() {
        "status" => {
            ...
            println!(
                "config_ready={} service_token_enrolled={} runtime_ready={} cached_installation_tokens={}",
                status.config_ready,
            ...
            Ok(
                if status.config_ready && status.service_token_enrolled && status.runtime_ready {
                    0
                } else {
                    1
                },
...
fn main() {
    ...
    match result {
        ...
        Err(error) => {
            if program == "git-credential-dev-auth"
