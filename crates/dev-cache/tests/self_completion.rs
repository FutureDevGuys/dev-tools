...
fn self_completion_bash_resolves_hyphenated_subcommand() {
    ...
    let output = Command::new(assert_cmd::cargo::cargo_bin!("dev-cache"))
        .env_clear()
...
read -r -a registration <<< "$(complete -p dev-cache)"
function_name=
...
COMP_WORDS=(dev-cache build-info --j)
COMP_CWORD=2
"$function_name" dev-cache --j build-info
printf '%s\n' "${COMPREPLY[@]}"
"#;
